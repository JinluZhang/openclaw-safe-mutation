...
- plan 会冻结 `readInvocation`、`writeInvocation`、`verifyInvocation` 和最终 `writePayload`。
  ...
  "protectedMutations": [
    {
      ...
      "match": {
        ...
        "resourceFlag": "--poiid",
        "mutableFlags": {
          "--tier-1-threshold": "tier_1_threshold",
          "--tier-1-discount": "tier_1_discount",
          "--remark": "remark"
        }
      },
      "read": {
...
如果 `protectedMutations` 省略，插件使用内置的 `mock-full-reduction.exec` binding。实际生产接入应显式配置自己的 binding。
...
- `match.mutableFlags`：CLI flag 到 `ParameterCatalog.fieldId` 的显式映射。未知 flag 会 fail closed。
...
1. 为写入口增加一条 binding，明确 `match`、目标 ID、字段映射和 `read`。
2. 确保所有 `mutableFlags` 指向已注册的 `ParameterCatalog.fieldId`。
